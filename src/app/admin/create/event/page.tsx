...
function toFirestoreTimestamp(local: string) {
...
const inputCls = 'bg-zinc-900 border border-zinc-700 rounded-md px-3 py-2 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:ring-1 focus:ring-zinc-600 transition-colors';
const inputErrCls = 'bg-zinc-900 border border-red-500/50 rounded-md px-3 py-2 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:ring-1 focus:ring-red-500/50 transition-colors';
const selectCls = `${inputCls} cursor-pointer`;
const disabledCls = 'bg-zinc-800 border border-zinc-700 rounded-md px-3 py-2 text-sm text-zinc-500 cursor-not-allowed';
...
function validate(form: FormData): Partial<Record<keyof FormData | '_global', string>> {
  const errors: Partial<Record<keyof FormData | '_global', string>> = {};
  ...
  if (total > 0) {
    ...
    if (sum > total) {
      errors.firstPrize = `Distribution (₹${sum}) exceeds pool (₹${total}).`;
    }
  }
...
export default function CreateEventPage() {
  ...
  const [fieldErrors, setFieldErrors] = useState<Partial<Record<keyof FormData | '_global', string>>>({});
  const [submitting, setSubmitting] = useState(false);
  const [serverError, setServerError] = useState('');
  // Track whether slug was manually edited so auto-fill stops overwriting it
  ...
  function set<K extends keyof FormData>(key: K, value: FormData[K]) {
    setForm((prev) => {
      ...
      // Auto-fill slug from event name unless manually edited or random mode
      if (key === 'eventName' && !slugManuallyEdited.current && !prev.useRandomSlug) {
        next.slug = toSlug(value as string);
      }

      // When enabling random slug, generate one immediately
      if (key === 'useRandomSlug' && value === true) {
        next.slug = randomSlug();
        slugManuallyEdited.current = false;
      }
      // When disabling random slug, revert to name-based slug
      if (key === 'useRandomSlug' && value === false) {
        next.slug = toSlug(prev.eventName);
        slugManuallyEdited.current = false;
      }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const startTs = toFirestoreTimestamp(form.startDate);
      const endTs   = toFirestoreTimestamp(form.endDate);

      const payload: Partial<Event> & { slug?: string } = {
        ...
        startDate:          startTs as unknown as Event['startDate'],
        endDate:            endTs   as unknown as Event['endDate'],
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <Field label="Slug" hint="Used in event URLs. Auto-generated from name.">
          <div className={`flex rounded-md border overflow-hidden focus-within:ring-1 focus-within:ring-zinc-600 ${fieldErrors.slug ? 'border-red-500/50' : 'border-zinc-700'}`}>
            <span className="px-3 py-2 text-sm text-zinc-500 bg-zinc-800 border-r border-zinc-700 flex-shrink-0 select-none">
              /events/
            </span>
            <input
              type="text"
              value={form.slug}
              disabled={form.useRandomSlug}
              onChange={(e) => {
          ...
          <div className="flex items-center gap-2 mt-1.5">
            ...
            {form.useRandomSlug && (
              <button
                type="button"
                onClick={() => setForm((p) => ({ ...p, slug: randomSlug() }))}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Status" hint={form.status === 'upcoming' ? 'Start date must be in the future' : form.status === 'ongoing' ? 'Start in past, end in future' : 'End date must be in the past'}>
        ...
        <Field label="Thumbnail URL" hint="Direct URL to the event banner image.">
          <input
            type="url"
            value={form.eventThumbnail}
            onChange={(e) => set('eventThumbnail', e.target.value)}
            className={inputCls}
            placeholder="https://..."
          />
        </Field>
        ...
        {form.isTeamEvent && (
          <div className="grid grid-cols-2 gap-4 mt-1">
            <Field label="Min team members" error={fieldErrors.minTeamMembers}>
              <input type="number" min={1} value={form.minTeamMembers} onChange={(e) => set('minTeamMembers', e.target.value)} className={fieldErrors.minTeamMembers ? inputErrCls : inputCls} />
            </Field>
            <Field label="Max team members">
              <input type="number" min={1} value={form.maxTeamMembers} onChange={(e) => set('maxTeamMembers', e.target.value)} className={inputCls} />
            </Field>
          </div>
        )}
        ...
        {form.isFemaleMandatory && (
          <Field label="Minimum female members required" error={fieldErrors.minFemaleRequired}>
            <input type="number" min={1} value={form.minFemaleRequired} onChange={(e) => set('minFemaleRequired', e.target.value)} className={`${fieldErrors.minFemaleRequired ? inputErrCls : inputCls} mt-1`} />
          </Field>
        )}
        ...
        <Field label="Total prize pool (₹)" hint="Set to 0 if no prize money.">
          <input type="number" min={0} value={form.prizeMoney} onChange={(e) => set('prizeMoney', e.target.value)} className={inputCls} />
        </Field>
          ...
          <>
            <div className="grid grid-cols-3 gap-4">
              <Field label="🥇 1st prize (₹)" error={fieldErrors.firstPrize}>
                <input type="number" min={0} value={form.firstPrize} onChange={(e) => set('firstPrize', e.target.value)} className={fieldErrors.firstPrize ? inputErrCls : inputCls} />
              </Field>
              <Field label="🥈 2nd prize (₹)">
                <input type="number" min={0} value={form.secondPrize} onChange={(e) => set('secondPrize', e.target.value)} className={inputCls} />
              </Field>
              <Field label="🥉 3rd prize (₹)">
                <input type="number" min={0} value={form.thirdPrize} onChange={(e) => set('thirdPrize', e.target.value)} className={inputCls} />
              </Field>
            </div>
            {/* Live prize budget indicator */}
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-zinc-800 mt-2">
          ...
          <Link href="/admin/events" className="px-5 py-2 text-sm text-zinc-400 hover:text-zinc-100 transition-colors">
            Cancel
          </Link>
        </div>
